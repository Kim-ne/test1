...
// nhúng file vào vị trí cần thiết để sd lại : include (path)
    ...
    // // // Tìm user muốn sửa
    // $list = getListUser();
    // $user = $list[$_GET['username']] ?? null; // lấy ra user muốn xóa
...
<body>
    <form method="post" enctype="multipart/form-data">
        <div class="container mt-3">
            <?= $msg ?? '' ?>
            <div class="col-xl-4">
                <h4 class="text-center my-4 text-uppercase">Thông tin user</h4>
                ...
                <div class="form-check form-check-inline">
                    <label class="form-check-label">
                        <input class="form-check-input" type="checkbox"<?=$_SESSION['user']['status']== 0? 'checked':''?> name="status" id="" value="0">Khóa
                    </label>
                    <label class="form-check-label ml-3">
                        <input class="form-check-input" type="checkbox" <?=$_SESSION['user']['status']== 1? 'checked':''?> name="status" id="" value="1">Kích hoạt
                    </label>
                </div>
                <div class="form-group">
                    <label for="">Mã cookie</label>
                    <input type="text" class="form-control" name="key_cookie" id="" placeholder=""
                        value="<?=$_SESSION['user']['key_cookie']?>">
                </div>
                <div class="row">
